...
def effGDC(s1,s2):#function to test all three algs for time and space efficiency and print the results
    print ('\nTesting Algorythem1-Euclids Algorithm')#test the first alg
    t0=time.time()
    GCD=alg1(s1,s2)
    t1=time.time()
    T=t1-t0
    print('The GCD: '+str(GCD)+' Was calculated in time: '+str(T)+' seconds memory used displayed above')

    print ('\nTesting Algorythem2-Consecutive Integer Checking Algorithm')#test the second alg
    t0=time.time()
    alg2(s1,s2)
    t1=time.time()
    T=t1-t0
    ...
    print ('\nTesting Algorythem3-Middle School Procedure')#test the third alg
    t0=time.time()
    alg3(s1,s2)
    t1=time.time()
    T=t1-t0
